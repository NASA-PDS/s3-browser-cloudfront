...
export function render() {
    ...
    // Use Bootstrap table
    $("#files table").addClass("table table-hover").wrap("<div class='table-responsive'>");
    ...
    $("#files table").prependIfNotExist("tbody", "<tbody/>").append($("tr"));

    // Move table header in <thead>
    $("#files table").prependIfNotExist("thead", "<thead/>").append($("tr").first());
    ...
    if ("C" in args && args["C"] in sort_element_by_arg
        ...
        var icon_html = create_icon(order.classes, title);
        $("a", element.element).append(icon_html);
    }
    ...
    const icon_class_by_type = {
        ...
        "back": {
            ...
            classes: "fa-fw fas fa-arrow-left",
        },
        "directory": {
            title: "Directory",
    ...
    $("#files table > tbody tr").each(function(index) {
        ...
        var img_alt = $("img", icon).attr("alt");
        if (img_alt === undefined) {
            return;
        }

        var type = img_alt.replace(/[\[\] ]+/g, "");
        ...
        var icon_html = create_icon(icon_class.classes, icon_class.title); 
        icon.html(icon_html);
